...
<%@page import="com.tao.model.Collection"%>
...
Collection collection = (Collection)request.getAttribute("detail_collection");
...
int btn_state = collection.getActiveNum() - collection.getCurrentNum();
Date deadline = collection.getDeadline();
					...
					<div>
						<table class="table table-bordered table-hover">
							...
							<tr>
								...
								<td>
									<span class="text-muted" style="font-size: 20px"><%=collection.getActiveNum()-collection.getCurrentNum() %></span>
								</td>
							</tr>
							<tr>
								<td>
						...
						<form id = "trade_form"style = "float:left"action="<%=request.getContextPath() %>/BookServlet" method="post">
							...
							<input id ="buy_btn"type="submit" class="btn btn-danger"onclick = "buy('<%=session.getAttribute("sessionUser") != null  %>')" value="购买">
							<input id ="book_btn"style ="color:grey "type="submit" class="btn btn-danger"onclick = "buy('<%=session.getAttribute("sessionUser") != null  %>')" value="预定">
						</form>
...
<script>
    ...
    window.onload=set_btn("<%=btn_state%>");
